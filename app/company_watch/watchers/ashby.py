...
_SOURCE_PREFIX = "target_company:ashby"
_RESPONSE_SNIPPET_LIMIT = 400


@dataclass(frozen=True)
class AshbyCompanyError:
    company_name: str
    message: str
    slug: str | None = None
...
@dataclass(frozen=True)
class AshbyWatchResult:
    vacancies: list[NormalizedVacancy]
    errors: list[AshbyCompanyError]
    raw_fetched: int = 0


class AshbyCollectionError(Exception):
    def __init__(
        self,
        message: str,
        *,
...
class AshbyTargetWatcher:
    ...
    def watch(self, companies: TargetCompany | Sequence[TargetCompany]) -> AshbyWatchResult:
        vacancies: list[NormalizedVacancy] = []
        errors: list[AshbyCompanyError] = []
        raw_fetched = 0
        with _build_client(
            ...
            for company in _as_company_list(companies):
                if not is_ashby_target(company):
                    continue
                try:
                    ...
                    message = str(exc).strip() or f"Ashby watch failed for '{company.name}'."
                    logger.warning("Ashby watch failed for '%s': %s", company.name, message)
                    errors.append(
                        AshbyCompanyError(
        ...
        return AshbyWatchResult(vacancies=vacancies, errors=errors, raw_fetched=raw_fetched)
...
def is_ashby_target(company: TargetCompany) -> bool:
    watcher_type = company.watcher_type.strip().lower()
    ats = (company.ats or "").strip().lower()
    return watcher_type == "ashby" or ats == "ashby"


def ashby_jobs_endpoint(slug: str) -> str:
    return f"https://api.ashbyhq.com/posting-api/job-board/{slug}"


def resolve_ashby_slug(company: TargetCompany) -> str:
    ...
    raise ValueError(f"Cannot resolve Ashby board slug for company: {company.name}")


def fetch_ashby_jobs(slug: str, *, client: httpx.Client) -> list[object]:
    endpoint = ashby_jobs_endpoint(slug)
    ...
    if not isinstance(jobs, list):
        logger.warning("Unexpected Ashby payload for board '%s'", slug)
        return []
    return jobs


def ashby_job_to_normalized(
    item: object,
    *,
    source: str,
    ...
    if not title or not url or not external_id:
        return None
    location = str(item.get("locationName") or item.get("location") or "").strip() or None
    employment = str(item.get("employmentType") or "").strip() or None
    description = str(item.get("descriptionPlain") or "").strip()
    if not description:
        description = clean_html_to_text(str(item.get("descriptionHtml") or item.get("description") or ""))
    if not description:
        description = title
    published = str(item.get("publishedAt") or item.get("publishedDate") or "") or None
...
def _request_error(
    ...
    *,
    ...
    if isinstance(status_code, int):
        message = f"Ashby board '{slug}' request failed ({status_code})."
    else:
        message = f"Ashby board '{slug}' request failed ({error_type})."
    snippet = None
